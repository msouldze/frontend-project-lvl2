    ...
    .map(([key, value]) => {
      if (_.has(data1, key) && _.has(data2, key)) {
        if ((_.isObject(data1[key]) && _.isObject(data2[key])) || data1[key] === data2[key]) {
          return [' ', `${key}`, getDiff(data1[key], data2[key])];
        }
        if (data1[key] !== data2[key]) {
          if (data1[key] === value) {
            return ['-', `${key}`, getDiff(data1[key], data1[key])];
          }
          return ['+', `${key}`, getDiff(data2[key], data2[key])];
        }
      }
      return (!_.has(data1, key) && _.has(data2, key)) ? ['+', `${key}`, getDiff(data2[key], data2[key])] : ['-', `${key}`, getDiff(data1[key], data1[key])];
    });
